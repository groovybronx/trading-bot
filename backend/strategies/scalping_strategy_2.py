...
def calculate_supertrend(
    ...
    try:
        ...
        if not all(
            ...
            for col in ["high", "low", "atr"]
        ):
            logger.error(
                "Supertrend: Colonnes 'high', 'low' ou 'atr' manquantes ou non numériques."
            )
        ...
        for i in range(1, len(df)):
            ...
            if prev_st_dir == 1:  # Tendance haussière précédente
                if curr_close > prev_st:  # Reste au-dessus
                    ...
                    st_val = (
                        max(prev_st, curr_lower)
                        if pd.notna(prev_st) and pd.notna(curr_lower)
                        else (prev_st if pd.notna(prev_st) else curr_lower)
                    )
                ...
                if curr_close < prev_st:  # Reste en dessous
                    ...
                    st_val = (
                        min(prev_st, curr_upper)
                        if pd.notna(prev_st) and pd.notna(curr_upper)
                        else (prev_st if pd.notna(prev_st) else curr_upper)
                    )
    ...
    try:
        ...
        for col in required_numeric_cols:
            ...
            if col not in klines_df.columns:
                logger.error(
                    f"SCALPING2 Indicators: Colonne requise '{col}' manquante."
                )
            ...
            if not pd.api.types.is_numeric_dtype(klines_df[col]):
                ...
                if not pd.api.types.is_numeric_dtype(klines_df[col]):
                    logger.error(
                        f"SCALPING2 Indicators: Colonne '{col}' non numérique même après conversion."
                    )
                    cols_to_add = [
                        "atr",
                        "upperband",
        ...
        required_len = max(
            ...
            config.get("STOCH_K_PERIOD", 14)
            + config.get("STOCH_D_PERIOD", 3),  # Approximatif
            config.get("SCALPING_RSI_PERIOD", 7),
            config.get("SUPERTREND_ATR_PERIOD", 3)
            + 1,  # Besoin d'une période de plus pour ST
        )

        if len(klines_df) < required_len:
            logger.warning(
                f"SCALPING2: Données insuffisantes ({len(klines_df)}/{required_len}) pour indicateurs."
            )
        ...
        # RSI
        rsi_series = ta.rsi(
            klines_df["close"], length=config.get("SCALPING_RSI_PERIOD", 7)
        )
        ...
        if stoch is not None and not stoch.empty:
            ...
            k_col = next(
                (col for col in stoch.columns if col.startswith("STOCHk")), None
            )
            d_col = next(
                (col for col in stoch.columns if col.startswith("STOCHd")), None
            )
            klines_df["stoch_k"] = (
                stoch[k_col] if k_col and k_col in stoch.columns else np.nan
            )
            klines_df["stoch_d"] = (
                stoch[d_col] if d_col and d_col in stoch.columns else np.nan
            )
        else:
        ...
        if bbands is not None and not bbands.empty:
            ...
            klines_df["bb_upper"] = (
                bbands[u_col] if u_col and u_col in bbands.columns else np.nan
            )
            klines_df["bb_middle"] = (
                bbands[m_col] if m_col and m_col in bbands.columns else np.nan
            )
            klines_df["bb_lower"] = (
                bbands[l_col] if l_col and l_col in bbands.columns else np.nan
            )
        else:
        ...
        # Volume moyen
        volume_sma = ta.sma(
            klines_df["volume"], length=config.get("VOLUME_MA_PERIOD", 20)
        )
        ...
        logger.error(
            f"Erreur dans le calcul des indicateurs SCALPING2: {e}", exc_info=True
        )
...
def check_long_conditions(row: pd.Series, prev_row: pd.Series) -> Tuple[bool, str]:
    ...
    try:
        ...
        if any(pd.isna(row.get(col)) for col in required_cols) or any(
            pd.isna(prev_row.get(col)) for col in required_cols
        ):
        ...
        if not isinstance(bb_lower_val, Decimal):
            return False, "Valeur bb_lower invalide"
        if row["close"] < bb_lower_val or row["close"] > bb_lower_val * Decimal(
            "1.01"
        ):  # Doit être proche ou au-dessus de la bande inf
...
def check_short_conditions(row: pd.Series, prev_row: pd.Series) -> Tuple[bool, str]:
    ...
    try:
        ...
        if not isinstance(bb_upper_val, Decimal):
            return False, "Valeur bb_upper invalide"
        if row["close"] > bb_upper_val or row["close"] < bb_upper_val * Decimal(
            "0.99"
        ):  # Doit être proche ou en dessous de la bande sup
...
def calculate_dynamic_sl_tp(
    ...
    try:
        ...
        if not isinstance(atr_value, Decimal) or atr_value <= 0:
            logger.warning(
                f"ATR invalide ({atr_value}) pour calcul SL/TP dynamique. Utilisation SL/TP fixes."
            )
        ...
        atr_sl_frac = (
            atr_value * Decimal("2")
        ) / entry_price  # 2 * ATR comme fraction du prix
        ...
        if side == "BUY":
            ...
            if isinstance(recent_low, Decimal) and recent_low > 0:
                sl_price = min(
                    sl_price, recent_low * (Decimal(1) - Decimal("0.001"))
                )  # Légèrement sous le plus bas

            # TP pour LONG: au-dessus du prix d'entrée
            risk_amount = entry_price - sl_price  # Risque en $ par unité
            if risk_amount <= 0:
                raise ValueError("Risque calculé non positif pour BUY")
            ...
            if isinstance(recent_high, Decimal) and recent_high > 0:
                sl_price = max(
                    sl_price, recent_high * (Decimal(1) + Decimal("0.001"))
                )  # Légèrement au-dessus du plus haut

            # TP pour SHORT: sous le prix d'entrée
            risk_amount = sl_price - entry_price  # Risque en $ par unité
            if risk_amount <= 0:
                raise ValueError("Risque calculé non positif pour SELL")
            tp1_price = entry_price - (risk_amount * tp1_ratio)
            tp2_price = entry_price - (risk_amount * tp2_ratio)
        ...
        logger.error(
            f"Erreur calcul SL/TP dynamique: {e}", exc_info=False
        )  # exc_info=False pour moins de bruit
        ...
        if entry_price <= 0:  # Si même le prix d'entrée est invalide, retourner 0
            logger.error(
                "Prix d'entrée invalide, impossible de calculer SL/TP fallback."
            )
...
def check_exit_conditions(
    ...
    try:
        ...
        try:
            entry_price = (
                Decimal(str(entry_price_str)) if entry_price_str is not None else None
            )
            sl_price = Decimal(str(sl_price_str)) if sl_price_str is not None else None
            tp1_price = (
                Decimal(str(tp1_price_str)) if tp1_price_str is not None else None
            )
            highest_price = (
                Decimal(str(highest_price_str))
                if highest_price_str is not None
                else entry_price
            )  # Default to entry
            lowest_price = (
                Decimal(str(lowest_price_str))
                if lowest_price_str is not None
                else entry_price
            )  # Default to entry
        except (InvalidOperation, TypeError):
            logger.error(
                f"SCALPING2 Exit Check: Erreur conversion Decimal données position: {position_data}"
            )
        ...
        if (
            ...
            logger.error(
                f"SCALPING2 Exit Check: Données de position invalides après conversion: Side={side}, Entry={entry_price}, SL={sl_price}, TP1={tp1_price}"
            )
            return (
                True,
                "Données de position invalides",
            )  # Forcer sortie si données corrompues

        # 1. Vérification SL / TP
        if side == "BUY":
            if current_price <= sl_price:
                return True, f"Stop Loss déclenché @ {sl_price:.8f}"
            if current_price >= tp1_price:
                return True, f"Take Profit 1 déclenché @ {tp1_price:.8f}"
        else:  # SELL
            if current_price >= sl_price:
                return True, f"Stop Loss déclenché @ {sl_price:.8f}"
            if current_price <= tp1_price:
        ...
        if trailing_stop_frac > 0:
            if side == "BUY":
                ...
                trailing_trigger_price = highest_price * (
                    Decimal(1) - trailing_stop_frac
                )
                ...
                if trailing_trigger_price > entry_price:
                    if current_price < trailing_trigger_price:
                        return (
                            True,
                            f"Trailing Stop (LONG) déclenché @ {trailing_trigger_price:.8f}",
                        )
            else:  # SELL
                trailing_trigger_price = lowest_price * (
                    Decimal(1) + trailing_stop_frac
                )
                ...
                if trailing_trigger_price < entry_price:
                    if current_price > trailing_trigger_price:
                        return (
                            True,
                            f"Trailing Stop (SHORT) déclenché @ {trailing_trigger_price:.8f}",
                        )
        ...
        return (
            True,
            f"Erreur technique exit check: {e}",
        )  # Forcer sortie en cas d'erreur
...
def prepare_order_params(
    ...
    """
    Prépare les paramètres d'ordre pour SCALPING2 en respectant le sizing (risk/capital).
    """
    from utils.order_utils import format_quantity, get_min_notional, check_min_notional

    try:
        risk_per_trade_frac = current_config.get("RISK_PER_TRADE", Decimal("0.01"))
        capital_allocation_frac = current_config.get(
        ...
        if risk_per_unit <= 0:
            logger.warning(
                f"SCALPING2 Entry: Risque par unité nul ou négatif (SL={stop_loss_price:.8f}, Entry={entry_price:.8f})."
            )
            return None
        ...
        if formatted_quantity is None or formatted_quantity <= 0:
            logger.warning(
                f"SCALPING2 Entry: Quantité ({quantity_unformatted:.8f}) invalide après formatage LOT_SIZE."
            )
            return None
        min_notional = get_min_notional(symbol_info)
        if not check_min_notional(formatted_quantity, entry_price, min_notional):
            logger.warning(
                f"SCALPING2 Entry: Notionnel ({formatted_quantity * entry_price:.4f}) < MIN_NOTIONAL ({min_notional:.4f}) après formatage Qty. Ordre non placé."
            )
            return None
        order_params = {
            "side": side,
        ...
        logger.info(
            f"SCALPING2 Entry: Préparation ordre {side} MARKET Qty={formatted_quantity}"
        )
        return order_params
    except Exception as e:
        logger.error(
            f"SCALPING2 Entry: Erreur sizing/préparation ordre: {e}", exc_info=True
        )
        return None
