...
# --- Utilitaires ---
from websocket_utils import broadcast_state_update, broadcast_ticker_update # Assurez-vous que broadcast_ticker_update existe bien ici
from utils.order_utils import format_quantity, get_min_notional

# --- Logique Stratégie Spécifique ---
from strategies.swing_strategy import calculate_indicators_and_signals as swing_calculate, \
                                      check_entry_conditions as swing_check_entry, \
                                      check_exit_conditions as swing_check_exit
# MODIFIÉ: Import de scalping_strategy (suppression de check_limit_order_timeout)
from strategies.scalping_strategy import check_entry_conditions as scalping_check_entry, \
...
def execute_entry(order_params: Dict[str, Any]):
    ...
    try:
        ...
        if order_result and order_result.get('orderId'):
            ...
            logger.error(f"ENTRY Order Placement FAILED (via API) for {symbol}. No valid result/orderId.")
        ...
        logger.error(f"CRITICAL Error during ENTRY order placement for {symbol}: {e}", exc_info=True)
        # MODIFIÉ: Appel update_state avec un dictionnaire
        state_manager.update_state({
            "status": "ERROR",
            "open_order_id": None,
...
def execute_exit(reason: str):
    ...
    if not is_in_position or symbol_quantity_held <= 0 or not symbol or not base_asset:
        logger.warning(f"execute_exit called (Reason: {reason}) but state indicates not in position or invalid quantity for {symbol}.")
        return

    logger.info(f"Attempting EXIT for {symbol} (Reason: {reason}). Held Qty: {symbol_quantity_held}")
    ...
    try:
        ...
        if not symbol_info:
             ...
             if symbol_info:
                 ...
                 logger.error(f"EXIT Order: Failed to get symbol info for {symbol}. Cannot format quantity. Aborting exit.")
                 return

        quantity_to_sell = format_quantity(symbol_quantity_held, symbol_info)

        if quantity_to_sell <= 0:
            logger.error(f"EXIT Order: Calculated quantity to sell is zero or negative ({quantity_to_sell}) after formatting for {symbol}. Held: {symbol_quantity_held}. Aborting exit.")
            # MODIFIÉ: Appel update_state avec un dictionnaire
            state_manager.update_state({
                "in_position": False,
                "entry_details": None,
        ...
        if order_result and order_result.get('orderId'):
            logger.info(f"EXIT Order Placement successful (via API): ID {order_result['orderId']}. Reason: {reason}")
            # MODIFIÉ: Appel update_state avec un dictionnaire
            state_manager.update_state({"status": "EXITING"})
            # --- FIN MODIFIÉ ---
            broadcast_state_update()
        else:
            logger.error(f"EXIT Order Placement FAILED (via API) for {symbol}. Reason: {reason}")
            # MODIFIÉ: Appel update_state avec un dictionnaire
        ...
        logger.error(f"CRITICAL Error during EXIT order placement for {symbol}: {e}", exc_info=True)
        # MODIFIÉ: Appel update_state avec un dictionnaire
...
def process_book_ticker_message(msg: Dict[str, Any]):
    ...
    try:
        if isinstance(msg, dict) and 's' in msg and 'b' in msg and 'a' in msg:
            ...
            if symbol != configured_symbol: return

            state_manager.update_book_ticker(msg)
            broadcast_ticker_update(msg) # Assurez-vous que cette fonction existe dans websocket_utils
            ...
            if strategy_type == 'SCALPING':
                if open_order_id:
                    ...
                    sl_tp_result = scalping_check_sl_tp(configured_symbol, entry_details, msg, current_config)
                    if sl_tp_result: # Si SL ou TP est retourné
                        threading.Thread(target=execute_exit, args=(f"Hit ({sl_tp_result})",), daemon=True).start()
                        return
                    ...
                    if depth_data and scalping_check_strategy_exit(configured_symbol, entry_details, msg, depth_data, current_config):
                        threading.Thread(target=execute_exit, args=("Signal Scalping Strategy Exit",), daemon=True).start()
                        return
                    ...
                    symbol_info = state_manager.get_symbol_info()
                    if not symbol_info:
                         logger.warning("SCALPING Entry Check: Symbol info non disponible.")
                         return
                    entry_order_params = scalping_check_entry(
                        configured_symbol, msg, depth_data,
                        current_config, current_state.get("available_balance", 0.0), symbol_info
                    )
                    if entry_order_params:
                        threading.Thread(target=execute_entry, args=(entry_order_params,), daemon=True).start()
...
def process_kline_message(msg: Dict[str, Any]):
    """Callback @kline: Met à jour historique et déclenche logique SWING."""
    try:
        if isinstance(msg, dict) and msg.get('e') == 'kline' and 'k' in msg:
            ...
            if symbol != configured_symbol or interval != configured_timeframe or strategy_type != 'SWING':
                return
            ...
            if is_closed:
                ...
                if len(full_kline_history) < required_len:
                    logger.info(f"Kline WS (SWING): History ({len(full_kline_history)}/{required_len}) insufficient.")
                    return
                ...
                if not is_in_position:
                    ...
                    entry_order_params = swing_check_entry(
                        latest_data, configured_symbol, current_config,
                        current_state.get("available_balance", 0.0), symbol_info
                    )
                    if entry_order_params:
                        threading.Thread(target=execute_entry, args=(entry_order_params,), daemon=True).start()
                    ...
                    if swing_check_exit(latest_data, configured_symbol):
                        threading.Thread(target=execute_exit, args=("Signal Indicateur SWING",), daemon=True).start()
...
def _handle_execution_report(data: dict):
    ...
    logger.info(f"Execution Report: ID={order_id}, ClientID={client_order_id}, Status={status}, Side={side}, Type={order_type}"
    ...
    if symbol:
        ...
        logger.warning("ExecutionReport: Cannot trigger history refresh, symbol missing.")
    ...
    if status not in ['NEW', 'PARTIALLY_FILLED'] and current_open_order_id == order_id:
        ...
        if status in ['CANCELED', 'REJECTED', 'EXPIRED'] and current_state.get("status") in ["ENTERING", "EXITING"]:
        ...
        try:
            ...
            if exec_qty > 0:
                ...
                if side == 'BUY' and not current_state.get("in_position"):
                    logger.info(f"ExecutionReport (FILLED BUY): Entering position via WS. OrderID={order_id}, AvgPrice={avg_price:.4f}, Qty={exec_qty}")
                    state_updates["in_position"] = True
                    state_updates["entry_details"] = {
                        "order_id": order_id, "avg_price": avg_price,
                        "quantity": exec_qty, "timestamp": order_time,
                    }
                    state_updates["status"] = "RUNNING"
                elif side == 'SELL' and current_state.get("in_position"):
                    logger.info(f"ExecutionReport (FILLED SELL): Exiting position via WS. OrderID={order_id}, AvgPrice={avg_price:.4f}, Qty={exec_qty}")
                    state_updates["in_position"] = False
                    state_updates["entry_details"] = None
                    state_updates["status"] = "RUNNING"
            else:
                logger.warning(f"ExecutionReport (FILLED): Order {order_id} has zero executed quantity?")
        except (ValueError, TypeError, ZeroDivisionError, InvalidOperation) as e:
            logger.error(f"ExecutionReport (FILLED): Error processing order {order_id}: {e}", exc_info=True)
