...
class StateManager:
    def __init__(self):
        ...
        self._bot_state: Dict[str, Any] = {
            ...
            "last_order_timestamp": None,  # Timestamp du dernier ordre d'entrée (ms)
        }
    ...
    def _calculate_required_klines(self, config_dict: Dict[str, Any]) -> int:
        ...
        if strategy_type == "SCALPING":
            return 1  # Pas besoin d'historique kline
        ...
        if strategy_type == "SCALPING2":
            periods = [
                ...
                config_dict.get("STOCH_K_PERIOD", 14)
                + config_dict.get("STOCH_D_PERIOD", 3),  # Approx
        ...
        with self._config_state_lock:
            if key:
                ...
                if isinstance(value, collections.deque):
                    return collections.deque(list(value), maxlen=value.maxlen)
    ...
    def update_state(self, updates: Dict[str, Any]):
        ...
        with self._config_state_lock:
            ...
            if (
                "status" in updates
                and self._bot_state.get("status") != updates["status"]
            ):
                logger.info(f"StateManager: Status changing -> {updates['status']}")
            if (
                "in_position" in updates
                and self._bot_state.get("in_position") != updates["in_position"]
            ):
                logger.info(
                    f"StateManager: Position changing -> {updates['in_position']}"
                )
                ...
                if not updates["in_position"]:
                    if self._bot_state.get("entry_details"):
                        logger.debug(
                            "StateManager: Clearing entry_details on exiting position."
                        )
            ...
            if self._bot_state.get("in_position") and ticker_for_trailing:
                current_price_str = ticker_for_trailing.get(
                    "c"
                )  # Utiliser le last price du ticker
                ...
                if current_price_str and entry_details:
                    try:
                        ...
                        if current_price > entry_details.get(
                            "highest_price", Decimal("-Infinity")
                        ):
                        ...
                        logger.warning(
                            f"Failed to update highest/lowest price from ticker: {current_price_str}"
                        )
    ...
    def _format_order_for_history(
        self, order_details: Dict[str, Any], strategy: Optional[str] = None
    ) -> Dict[str, Any]:
        """Formate un ordre brut (REST ou WS) pour l'historique, inclut la stratégie."""
        ...
        timestamp = (
            order_details.get("updateTime")
            or order_details.get("time")
            or order_details.get("T")
        ...
        if side == "SELL" and status == "FILLED" and entry_details_hist:
            try:
                ...
                exec_qty_sell = Decimal(
                    str(order_details.get("executedQty") or order_details.get("z", "0"))
                )
                quote_qty_sell = Decimal(
                    str(
                        order_details.get("cummulativeQuoteQty")
                        or order_details.get("Z", "0")
                    )
                )
                ...
                logger.warning(
                    f"Failed to calculate performance for order {order_id}",
                    exc_info=True,
                )

        # Déterminer la stratégie (priorité: paramètre, sinon config courante)
        if not strategy:
        ...
        simplified_order = {
            ...
            or order_details.get("o"),
            "origQty": str(order_details.get("origQty") or order_details.get("q", "0")),
            "executedQty": str(
                order_details.get("executedQty") or order_details.get("z", "0")
            ),
            "cummulativeQuoteQty": str(
                order_details.get("cummulativeQuoteQty") or order_details.get("Z", "0")
            ),
    ...
    def replace_order_history(
        self, new_raw_history: List[Dict[str, Any]], strategy: Optional[str] = None
    ):
        ...
        if isinstance(new_raw_history, list):
            for order in new_raw_history:
                simplified_new_history.append(
                    self._format_order_for_history(order, strategy)
                )
        else:
        ...
        try:
            with self._config_state_lock:
                logger.info(
                    f"Replacing internal order history with {len(simplified_new_history)} orders."
                )
                simplified_new_history.sort(
    ...
    def add_or_update_order_history(
        self, order_data: Dict[str, Any], strategy: Optional[str] = None
    ):
        """Ajoute ou met à jour un ordre dans l'historique."""
        order_id_to_update = str(
            order_data.get("orderId") or order_data.get("i", "N/A")
        )
        ...
        formatted_order = self._format_order_for_history(order_data, strategy)
        ...
        try:
            ...
            logger.error(
                f"Error updating history for order {order_id_to_update}: {e}",
                exc_info=True,
            )
            return
    ...
    def get_order_history(
        self, strategy: Optional[str] = None, session_only: bool = True
    ) -> List[Dict[str, Any]]:
        """Retourne une copie triée de l'historique des ordres, filtrée par stratégie et session si précisé."""
        with self._config_state_lock:
            # Retourner une copie pour éviter modification externe
            history = list(self._bot_state.get("order_history", []))
            if session_only:
                history = [
                    o for o in history if o.get("session_id") == self._session_id
                ]
            if strategy:
                history = [o for o in history if o.get("strategy") == strategy]
            return history

    def clear_order_history(self):
        """Vide l'historique des ordres et démarre une nouvelle session."""
        with self._config_state_lock:
            self._bot_state["order_history"] = []
    ...
    def update_config_values(
        ...
        success, message, restart_recommended = config_manager.update_config(
            new_params_input
        )
        if success:
            updated_config = (
                config_manager.get_config()
            )  # Récupérer la nouvelle config interne
            state_updates = {}
    ...
    def replace_kline_history(self, klines: List[List[Any]]):
        ...
        with self._kline_lock:
            ...
            if current_maxlen != required_klines:
                logger.warning(
                    f"StateManager: Kline history maxlen ({current_maxlen}) differs from required ({required_klines}) during replace. Resizing."
                )
            ...
            logger.info(
                f"StateManager: Kline history replaced ({len(self._kline_history)}/{self._kline_history.maxlen})."
            )
    ...
    def resize_kline_history(self, new_maxlen: int):
        ...
        if self._kline_history.maxlen != new_maxlen:
            logger.info(
                f"StateManager: Resizing kline history from {self._kline_history.maxlen} to {new_maxlen}"
            )
    ...
    def update_depth(self, data: Dict[str, Any]):
        ...
        with self._realtime_lock:
            ...
            if "bids" in data:
                self._latest_depth_snapshot["bids"] = [b[:] for b in data["bids"]]
            if "asks" in data:
                self._latest_depth_snapshot["asks"] = [a[:] for a in data["asks"]]
            if "lastUpdateId" in data:
                self._latest_depth_snapshot["lastUpdateId"] = data["lastUpdateId"]
    ...
    def clear_realtime_data(self):
        ...
        with self._realtime_lock:
            ...
            logger.info(
                "StateManager: Real-time market data and symbol_info cache cleared."
            )
    ...
    def store_pending_order_details(
        self, client_order_id: str, details: Dict[str, Any]
    ):
    ...
    def get_and_clear_pending_order_details(
        ...
        with self._pending_orders_lock:
            ...
            if details:
                logger.debug(
                    f"Retrieved and cleared pending details for ClientID {client_order_id}"
                )
            else:
                logger.warning(
                    f"No pending details found for ClientID {client_order_id} on get_and_clear."
                )
            return details
    ...
    def clear_pending_order_details(self, client_order_id: str):
        ...
        with self._pending_orders_lock:
            if client_order_id in self._pending_order_details:
                ...
                logger.debug(
                    f"Cleared pending details for ClientID {client_order_id} (e.g., due to failure)."
                )
    ...
    def save_persistent_data(self) -> bool:
        ...
        try:
            ...
            logger.exception(
                f"StateManager: Unexpected error saving persistent data: {e}"
            )
            return False
    ...
    def _load_persistent_data(self):
        ...
        try:
            ...
            if (
                ...
                with self._config_state_lock:
                    self._bot_state["in_position"] = state_data.get(
                        "in_position", False
                    )
                    loaded_entry_details_str = state_data.get("entry_details")

                    # Reconvertir les strings en Decimal pour entry_details
                    if self._bot_state["in_position"] and isinstance(
                        loaded_entry_details_str, dict
                    ):
                        try:
                            ...
                            for key, val_str in loaded_entry_details_str.items():
                                ...
                                if key in [
                                    ...
                                    entry_details_reloaded[key] = (
                                        val_str  # order_id, side
                                    )
                            self._bot_state["entry_details"] = entry_details_reloaded
                        except (ValueError, TypeError, InvalidOperation) as e:
                            logger.error(
                                f"StateManager: Error converting loaded entry_details: {e}. Resetting position."
                            )
                            self._bot_state["in_position"] = False
                            self._bot_state["entry_details"] = None
                    elif self._bot_state["in_position"]:
                        logger.warning(
                            "StateManager: In position state loaded, but entry_details missing/invalid. Resetting."
                        )
                        self._bot_state["in_position"] = False
                        self._bot_state["entry_details"] = None
                    else:
                        self._bot_state["entry_details"] = (
                            None  # Assurer None si pas en position
                        )
                    ...
                    for order in history_data:
                        if isinstance(order, dict) and "orderId" in order:
                            ...
                            logger.warning(
                                f"StateManager: Invalid order format in history ignored: {order}"
                            )

                    self._bot_state["order_history"] = restored_history
                    self._bot_state["order_history"].sort(
                        key=lambda x: x.get("timestamp", 0), reverse=True
                    )
                    max_len = self._bot_state.get("max_history_length", 100)
                    self._bot_state["order_history"] = self._bot_state["order_history"][
                        :max_len
                    ]
                    ...
                    logger.info(
                        f"StateManager: State (in_position={self._bot_state['in_position']}) and history ({len(self._bot_state['order_history'])} orders) restored."
                    )
            else:
                logger.error(
                    f"StateManager: Invalid format in {DATA_FILENAME}. State not restored."
                )
            ...
            logger.exception(
                f"StateManager: Unexpected error loading persistent data: {e}"
            )
        ...
        with self._config_state_lock:
            if self._bot_state["in_position"] and not self._bot_state["entry_details"]:
                logger.warning(
                    "StateManager: Post-load check failed ('in_position' True, 'entry_details' missing). Resetting."
                )
                self._bot_state["in_position"] = False

    def get_last_order_timestamp(self) -> Optional[int]:
        """Retourne le timestamp (ms) du dernier ordre d'entrée exécuté."""
        with self._config_state_lock:
            return self._bot_state.get("last_order_timestamp")

    def set_last_order_timestamp(self, timestamp_ms: int):
        """Met à jour le timestamp (ms) du dernier ordre d'entrée exécuté."""
        with self._config_state_lock:
