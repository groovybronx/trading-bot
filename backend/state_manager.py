...
class StateManager:
    def __init__(self):
        ...
        self._config_state_lock = (
            threading.Lock()
        )  # État principal, config, historique ordres
        ...
        self._symbol_info_cache: Optional[Dict[str, Any]] = (
            None  # Cache pour symbol_info
        )
    ...
    def _calculate_required_klines(self, config_dict: Dict[str, Any]) -> int:
        ...
        if strategy_type == "SCALPING2":
            # Pour SCALPING2, nous avons besoin d'assez de données pour les indicateurs:
            # - Supertrend (ATR): 3-5 périodes
            # - RSI: 7 périodes
            # - Stochastic: 14 périodes max
            # - Bollinger: 20 périodes
            # - Volume MA: 20 périodes
            return 30  # Buffer supplémentaire pour sécurité
    ...
    def _format_order_for_history(
        ...
        order_id_str = str(
            order_details.get("orderId") or order_details.get("i", "N/A")
        )
        ...
        simplified_order = {
            "timestamp": order_details.get("updateTime")
    ...
    def replace_order_history(self, new_raw_history: List[Dict[str, Any]]):
        ...
        if did_save:
            ...
            logger.error(
                "replace_order_history: Broadcast annulé car sauvegarde échouée."
            )
    ...
    def add_or_update_order_history(self, order_data: Dict[str, Any]):
        ...
        if order_id_to_update == "N/A":
            logger.warning(
                "add_or_update_order_history: Order ID manquant, impossible de mettre à jour."
            )
            return
        ...
        if order_side == "SELL" and order_status == "FILLED":
            ...
            with self._config_state_lock:
                if self._bot_state.get("in_position"):
                    entry_details_for_calc = self._bot_state.get(
                        "entry_details", {}
                    ).copy()
            ...
            if entry_details_for_calc:
                try:
                    entry_price = Decimal(
                        str(entry_details_for_calc.get("avg_price", "0"))
                    )
                    exec_qty_sell = Decimal(
                        str(order_data.get("executedQty") or order_data.get("z", "0"))
                    )
                    quote_qty_sell = Decimal(
                        str(
                            order_data.get("cummulativeQuoteQty")
                            or order_data.get("Z", "0")
                        )
                    )
                    ...
                    if entry_price > 0 and exec_qty_sell > 0:
                        ...
                        logger.info(
                            f"Performance calculated for SELL order {order_id_to_update}: {performance_pct:.4%}"
                        )
                    else:
                        logger.warning(
                            f"Cannot calculate performance for SELL {order_id_to_update}: Invalid entry price or sell quantity."
                        )
                except (
                    ValueError,
                    TypeError,
                    ...
                    logger.warning(
                        f"Error calculating performance for SELL order {order_id_to_update}: {e}"
                    )
        ...
        try:
            ...
            logger.error(
                f"Error during lock acquisition or history update for order {order_id_to_update}: {e}",
                exc_info=True,
            )
            return
    ...
    def update_depth(self, data: Dict[str, Any]):  # Renommé pour clarté
        ...
        with self._realtime_lock:
            if "bids" in data:
                self._latest_depth_snapshot["bids"] = data["bids"]
            if "asks" in data:
                self._latest_depth_snapshot["asks"] = data["asks"]
            ...
            if (
                ...
                with self._config_state_lock:
                    ...
                    if self._bot_state["in_position"] and isinstance(
                        loaded_entry_details, dict
                    ):
                        try:
                            loaded_entry_details["avg_price"] = float(
                                loaded_entry_details.get("avg_price", 0.0)
                            )
                            loaded_entry_details["quantity"] = float(
                                loaded_entry_details.get("quantity", 0.0)
                            )
                            loaded_entry_details["timestamp"] = int(
                                loaded_entry_details.get("timestamp", 0)
                            )
                        ...
                        logger.warning(
                            "StateManager: In position state loaded, but entry_details missing/invalid. Resetting position."
                        )
                        self._bot_state["in_position"] = False
                        self._bot_state["entry_details"] = None
                    else:
                    ...
                    for order in history_data:
                        if isinstance(order, dict) and "orderId" in order:
                            try:
                                ...
                                order["performance_pct"] = (
                                    float(perf_pct_str)
                                    if perf_pct_str is not None
                                    else None
                                )
                                restored_history.append(order)
                            except (ValueError, TypeError, InvalidOperation) as e:
                                logger.warning(
                                    f"StateManager: Error converting fields for order {order.get('orderId')} in history: {e}"
                                )
                                restored_history.append(order)
                        else:
                            logger.warning(
                    ...
                    if len(self._bot_state["order_history"]) > max_len:
                        self._bot_state["order_history"] = self._bot_state[
                            "order_history"
                        ][:max_len]
