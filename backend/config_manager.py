...
class ConfigManager:
    ...
    def update_config(self, new_params_input: Dict[str, Any]) -> Tuple[bool, str, bool]:
        """
        ...
        try:
            converted_new_params = self._convert_input_params_to_internal(
                new_params_input
            )
        ...
        is_valid, error_message, restart_recommended = self._validate_internal_config(
            config_candidate_internal,
            self._config,  # Passer l'ancienne config pour comparaison
        )
    ...
    def _convert_input_params_to_internal(
        self, params_input: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for key, value in params_input.items():
            ...
            try:
                if key in PERCENTAGE_KEYS_INPUT:
                    ...
                    converted_params[key] = str(value).lower() in (
                        "true",
                        "1",
                        "t",
                        "yes",
                ...
                raise ValueError(
                    f"Impossible de convertir '{key}' (valeur: '{value}', type: {type(value)}): {e}"
                )
    ...
    def _validate_internal_config(
        ...
        try:
            # Timeframe
            new_tf = str(
                config_to_validate.get("TIMEFRAME", "1m")
            )  # Utiliser TIMEFRAME
            ...
            if not isinstance(risk_frac, Decimal) or not (
                Decimal("0") < risk_frac <= Decimal("1")
            ):
                errors.append(
                    f"RISK_PER_TRADE (fraction) doit être > 0 et <= 1 (reçu: {risk_frac})"
                )

            # Capital Allocation (fraction)
            alloc_frac = config_to_validate.get("CAPITAL_ALLOCATION")
            if not isinstance(alloc_frac, Decimal) or not (
                Decimal("0") < alloc_frac <= Decimal("1")
            ):
                errors.append(
                    f"CAPITAL_ALLOCATION (fraction) doit être > 0 et <= 1 (reçu: {alloc_frac})"
                )

            # Stop Loss (fraction) - Plage élargie (0.1% à 20%)
            sl_frac = config_to_validate.get("STOP_LOSS_PERCENTAGE")
            if not isinstance(sl_frac, Decimal) or not (
                Decimal("0.001") <= sl_frac <= Decimal("0.20")
            ):
                errors.append(
                    f"STOP_LOSS_PERCENTAGE (fraction) doit être entre 0.001 (0.1%) et 0.20 (20%) (reçu: {sl_frac})"
                )

            # Take Profit 1 (fraction) - Plage élargie (0.1% à 50%)
            tp1_frac = config_to_validate.get("TAKE_PROFIT_1_PERCENTAGE")
            if not isinstance(tp1_frac, Decimal) or not (
                Decimal("0.001") <= tp1_frac <= Decimal("0.50")
            ):
                errors.append(
                    f"TAKE_PROFIT_1_PERCENTAGE (fraction) doit être entre 0.001 (0.1%) et 0.50 (50%) (reçu: {tp1_frac})"
                )

            # Take Profit 2 (fraction) - Plage élargie (0.1% à 50%)
            tp2_frac = config_to_validate.get("TAKE_PROFIT_2_PERCENTAGE")
            if not isinstance(tp2_frac, Decimal) or not (
                Decimal("0.001") <= tp2_frac <= Decimal("0.50")
            ):
                errors.append(
                    f"TAKE_PROFIT_2_PERCENTAGE (fraction) doit être entre 0.001 (0.1%) et 0.50 (50%) (reçu: {tp2_frac})"
                )
            # Vérifier que TP2 > TP1 seulement si TP2 est utilisé/pertinent et les deux sont valides
            if (
                isinstance(tp2_frac, Decimal)
                and isinstance(tp1_frac, Decimal)
                and tp2_frac <= tp1_frac
            ):
            ...
            if not isinstance(tsl_frac, Decimal) or not (
                Decimal("0.001") <= tsl_frac <= Decimal("0.10")
            ):
                errors.append(
                    f"TRAILING_STOP_PERCENTAGE (fraction) doit être entre 0.001 (0.1%) et 0.10 (10%) (reçu: {tsl_frac})"
                )

            # Time Stop (minutes)
            time_stop = config_to_validate.get("TIME_STOP_MINUTES")
            if not isinstance(time_stop, int) or not (
                1 <= time_stop <= 240
            ):  # Ex: 1min à 4h
                errors.append(
                    f"TIME_STOP_MINUTES doit être un entier entre 1 et 240 (reçu: {time_stop})"
                )
            ...
            if new_strategy == "SCALPING":
                ...
                if previous_config and depth_levels != previous_config.get(
                    "SCALPING_DEPTH_LEVELS"
                ):
                    restart_recommended = True

                depth_speed = str(
                    config_to_validate.get("SCALPING_DEPTH_SPEED", "1000ms")
                ).lower()
                if depth_speed not in ["100ms", "1000ms"]:
                    errors.append("SCALPING_DEPTH_SPEED: '100ms' ou '1000ms'.")
                # CORRECTION: Vérifier changement seulement si ce n'est pas l'initialisation
                if previous_config and depth_speed != previous_config.get(
                ...
                if not isinstance(spread_thresh, Decimal) or spread_thresh < 0:
                    errors.append(
                        "SCALPING_SPREAD_THRESHOLD (fraction) doit être >= 0."
                    )

                imbalance_thresh = config_to_validate.get(
                    "SCALPING_IMBALANCE_THRESHOLD"
                )
                # Permettre float ou Decimal ici
                if (
                    not isinstance(imbalance_thresh, (float, Decimal))
                    or Decimal(str(imbalance_thresh)) <= 0
                ):
                    errors.append(
            ...
            # --- Scalping 2 Params ---
                ...
                if (
                    not isinstance(
                        config_to_validate.get("SUPERTREND_ATR_PERIOD", 0), int
                    )
                    or config_to_validate.get("SUPERTREND_ATR_PERIOD", 0) <= 0
                ):
                    errors.append("SUPERTREND_ATR_PERIOD > 0")
                if (
                    not isinstance(
                        config_to_validate.get("SUPERTREND_ATR_MULTIPLIER", 0.0), float
                    )
                    or config_to_validate.get("SUPERTREND_ATR_MULTIPLIER", 0.0) <= 0
                ):
                    errors.append("SUPERTREND_ATR_MULTIPLIER > 0")
                if (
                    not isinstance(
                        config_to_validate.get("SCALPING_RSI_PERIOD", 0), int
                    )
                    or config_to_validate.get("SCALPING_RSI_PERIOD", 0) <= 1
                ):
                    errors.append("SCALPING_RSI_PERIOD > 1")
                if (
                    not isinstance(config_to_validate.get("STOCH_K_PERIOD", 0), int)
                    or config_to_validate.get("STOCH_K_PERIOD", 0) <= 0
                ):
                    errors.append("STOCH_K_PERIOD > 0")
                if (
                    not isinstance(config_to_validate.get("STOCH_D_PERIOD", 0), int)
                    or config_to_validate.get("STOCH_D_PERIOD", 0) <= 0
                ):
                    errors.append("STOCH_D_PERIOD > 0")
                if (
                    not isinstance(config_to_validate.get("STOCH_SMOOTH", 0), int)
                    or config_to_validate.get("STOCH_SMOOTH", 0) <= 0
                ):
                    errors.append("STOCH_SMOOTH > 0")
                if (
                    not isinstance(config_to_validate.get("BB_PERIOD", 0), int)
                    or config_to_validate.get("BB_PERIOD", 0) <= 1
                ):
                    errors.append("BB_PERIOD > 1")
                if (
                    not isinstance(config_to_validate.get("BB_STD", 0.0), float)
                    or config_to_validate.get("BB_STD", 0.0) <= 0
                ):
                    errors.append("BB_STD > 0")
                if (
                    not isinstance(config_to_validate.get("VOLUME_MA_PERIOD", 0), int)
                    or config_to_validate.get("VOLUME_MA_PERIOD", 0) <= 0
                ):
            ...
            # --- Swing Params ---
                ...
                if not isinstance(ema_s, int) or ema_s <= 0:
                    errors.append("EMA_SHORT_PERIOD doit être un entier > 0")
                if not isinstance(ema_l, int) or ema_l <= 0:
                    errors.append("EMA_LONG_PERIOD doit être un entier > 0")
                if not isinstance(ema_f, int) or ema_f <= 0:
                    errors.append("EMA_FILTER_PERIOD doit être un entier > 0")
                if not isinstance(rsi_p, int) or rsi_p <= 1:
                    errors.append("RSI_PERIOD doit être un entier > 1")
                if not isinstance(rsi_ob, int) or not (50 < rsi_ob <= 100):
                    errors.append("RSI_OVERBOUGHT doit être un entier > 50 et <= 100")
                if not isinstance(rsi_os, int) or not (0 <= rsi_os < 50):
                    errors.append("RSI_OVERSOLD doit être un entier >= 0 et < 50")
                if not isinstance(vol_p, int) or vol_p <= 0:
                    errors.append("VOLUME_AVG_PERIOD doit être un entier > 0")
                ...
                if (
                    isinstance(rsi_os, int)
                    and isinstance(rsi_ob, int)
                    and rsi_os >= rsi_ob
                ):
            ...
            if previous_config:  # Ne vérifier que si on met à jour une config existante
                for key in ["BINANCE_API_KEY", "BINANCE_API_SECRET", "SYMBOL"]:
                    ...
                    if key in config_to_validate and config_to_validate.get(
                        key
                    ) != previous_config.get(key):
                        errors.append(
                            f"La clé '{key}' ne peut pas être modifiée après le démarrage."
                        )
                        restart_recommended = True  # Forcer redémarrage si tentative

                if "USE_TESTNET" in config_to_validate and config_to_validate.get(
                    "USE_TESTNET"
                ) != previous_config.get("USE_TESTNET"):
                    errors.append(
                        "Le changement de USE_TESTNET nécessite un redémarrage."
                    )
                    restart_recommended = True

            # Ajouter validation ORDER_COOLDOWN_MS (doit être >= 0)
            cooldown = config_to_validate.get("ORDER_COOLDOWN_MS")
            if not isinstance(cooldown, int) or cooldown < 0:
                errors.append("ORDER_COOLDOWN_MS doit être un entier >= 0.")
        ...
        if errors:
            return (
                ...
                " / ".join(errors),
                False,
            )  # Ne pas recommander redémarrage si juste invalide
        else:
